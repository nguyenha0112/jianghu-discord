...
const rankingFile = path.join(dataDir, "baucua-ranking.json");
const historyFile = path.join(dataDir, "baucua-history.json");
...
function updateBauCuaRanking(userId, username, patch) {
...
function getBauCuaRanking(limit = 10) {
  const store = readJson(rankingFile);
  return Object.values(store.players)
    .filter((entry) => entry.wins > 0 || entry.games > 0 || entry.profitXu !== 0 || entry.bestWinXu > 0)
    .sort((a, b) => {
      if (b.wins !== a.wins) {
...
function addBauCuaHistoryEntry(entry) {
...
function getBauCuaHistory(limit = 10) {
  const store = readJson(historyFile);
